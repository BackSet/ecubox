...
**ECUBOX** es un sistema de gestión para acompañar envíos desde USA hacia Ecuador: consolidación tipo casillero, registro de paquetes por destinatario, control operativo (pesos, lotes, despachos) y **rastreo** para quien recibe el envío.

**Qué puedes hacer según tu perfil**

- Como **cliente o usuario del panel**: dar de alta destinatarios y paquetes, consultar información autorizada y seguir el flujo que tu cuenta permita.
- Como **operario o administrador**: usar módulos de carga, despachos, lotes, parámetros y catálogos según los permisos que te asignen.
...
- `DESTINATARIOS_READ` — Destinatarios  
- `PAQUETES_READ`, `PAQUETES_CREATE`, `PAQUETES_UPDATE`, `PAQUETES_DELETE`, `PAQUETES_PESO_WRITE` — Paquetes y pesos  
- `DESPACHOS_WRITE` — Despachos, lotes de recepción y entrada a **Parámetros** (menú Configuración)  
- `AGENCIAS_READ`, `AGENCIAS_DISTRIBUIDOR_READ`, `DISTRIBUIDORES_READ`, `MANIFIESTOS_READ` — Catálogos  
...
|-------|------------------------|
| **Principal** | Inicio, Agencia USA |
| **Operaciones** | Destinatarios, Paquetes, Cargar pesos, Estados de paquetes, Despachos, Lotes recepción |
| **Catálogos** | Agencias, Agencias distribuidor, Distribuidores, Manifiestos |
...
- **Permiso:** `DESTINATARIOS_READ` (y los de alta/edición si aplican).
...
- Registro de nuevos paquetes, edición y búsqueda por guía, referencia, destinatario, etc.
- **Asignar guía de envío:** botón visible para **ADMIN** y **OPERARIO**; abre la herramienta para vincular guías de envío a paquetes en bloque.

### Cargar pesos (`/cargar-pesos`)

- **Permiso:** `PAQUETES_PESO_WRITE`.
- Registro o actualización de peso de paquetes según el proceso operativo.
...
| Agencias distribuidor | `/agencias-distribuidor` | `AGENCIAS_DISTRIBUIDOR_READ` |
| Distribuidores | `/distribuidores` | `DISTRIBUIDORES_READ` |
...
### Agencia USA (`/agencia-eeuu`)

- En el grupo **Principal**; información institucional o de contacto según lo cargado en la aplicación.
...
| **Agencia USA** | Texto con dirección, horarios u otros datos que debe ver el cliente. |
...
Si editas texto en WhatsApp o Agencia USA e intentas **cambiar de pestaña o volver al menú** sin guardar, la aplicación puede pedirte **confirmación** para no perder cambios.
...
| Término | Significado |
|---------|-------------|
| **Paquete** | Unidad de envío registrada con guía y, en su caso, peso y destinatario. |
| **Guía / número de guía** | Identificador del envío para rastreo y operación. |
| **Guía de envío** | Referencia adicional usada en recepción o consolidación (según proceso). |
| **Destinatario** | Persona o entidad en destino asociada a los paquetes. |
