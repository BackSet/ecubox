...
- Frontend: conserva TanStack Router/Query, servicios `openapi-fetch`, stores Zustand y esquemas Zod existentes.
- Frontend (cliente HTTP): **`openapi-fetch` es el único cliente HTTP** (Axios retirado en MVP 6/8). `src/lib/api/openapi-client.ts` expone `openapiClient` (autenticado) y `openapiPublicClient` (público, sin token ni redirección 401) más `unwrap`/`ensureOk`/`ApiError` (lanzan en fallos HTTP, compatibles con `getApiErrorMessage`/`getApiStatus`). Auth/errores se comparten vía `http-feedback.ts` (no dupliques token, 401, toasts ni abort). Toda la capa de servicios usa `openapi-fetch` salvo `tracking.service.ts` (que usa `fetch` crudo por su UX bespoke 404/429). Para query params multivalor (`List<...>`) los clientes usan `querySerializer` comma-style (`estado=a,b`); descargas con `parseAs:'blob'`. Al añadir/editar un servicio: usa el cliente público para endpoints públicos, conserva firmas y tipos de dominio (contrato laxo; casts localizados al límite, nunca `as any`), mantén `AbortSignal` donde exista, compara ruta/param contra el contrato y **no cambies el wire** (payloads/filtros/endpoints): ante una discrepancia, presérvala con un cast documentado y repórtala como cambio funcional aparte. Los tipos viven en `src/lib/api/generated/schema.d.ts` (autogenerado, no editar a mano); regenera con `npm run api:generate`. `tsconfig` tiene `strict` + `noUncheckedIndexedAccess`; `exactOptionalPropertyTypes` está **aplazado** (impacto ~266 errores).
...
- Textos legales públicos: deben usar lenguaje claro, incluir aviso de revisión por asesoría legal antes de publicación definitiva y no prometer controles, proveedores, consentimientos, cookies ni cumplimiento que no estén evidenciados en el código/configuración. Las rutas canónicas son `/terminos-y-condiciones`, `/politica-de-privacidad` y `/politica-de-cookies`.
...
- Frontend (marca en PDFs): los documentos PDF/exportables deben usar los assets oficiales de marca mediante `src/lib/pdf/brand-logo.ts` y los DataURLs generados por `scripts/generate-brand-assets.mjs`; no redibujes lockups con texto `ec`, `ECUBOX` o el lema.
...
- **Flyway se ejecuta automáticamente al arrancar el backend en cualquier perfil** (`spring.flyway.enabled=true`, sin guardia de entorno). Un `git push` **no** toca la DB de ningún entorno real. El CI ejecuta las migraciones de producción contra un **PostgreSQL efímero de Testcontainers** (prueba de integración, contenedor descartable), nunca contra una DB de entorno; el arranque/reinicio del backend de un entorno sí **aplica las migraciones pendientes a la DB de ese entorno**. Tenlo en cuenta antes de promover migraciones a `main`.
- Para validar SQL de una migración sin un arranque completo: ejecútala dentro de `BEGIN … ROLLBACK` con `psql` contra un clon/transacción reversible de dev (sourcing `.env`, nunca exponiendo la contraseña). Para validar la cadena completa de migraciones de forma reproducible existe el arnés de integración Testcontainers (`*IntegrationTest`, requiere Docker).
