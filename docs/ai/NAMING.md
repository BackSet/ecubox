...
| Conecta · Envía · Llega | Otros lemas en copy de marca | Lema de marca (rebranding); símbolo = monograma «ec» enlazado (conexión/infinito) | **Verificado en Git**: `docs/branding/IDENTIDAD_VISUAL.md`, `scripts/brand-glyphs.mjs` |
...
| Punto de retiro del courier | Retiro en agencia, punto de entrega cuando describe la modalidad | Despacho `tipo_entrega = AGENCIA_COURIER_ENTREGA`; la entidad sigue siendo `AgenciaCourierEntrega` | **Verificado en Git**: rastreo público y `TipoEntrega` |
| Entrega a domicilio | Delivery, entrega domiciliaria | Despacho `tipo_entrega = DOMICILIO` | **Verificado en Git**: rastreo público y `TipoEntrega` |
...
- **Peso pendiente**: un peso nulo es dato desconocido, no cero. En despachos y liquidaciones se permite incluir paquetes sin peso, se muestra advertencia no bloqueante y los totales/sugeridos se rotulan como **parciales** hasta registrar el peso.
- **Filtros de listado de consolidados**: **«Requiere atención»** (`requiereAtencion`, para consolidados con paquetes en flujo alterno o sin estado), **«Estados mixtos»** (`estadosMixtos`, para consolidados con paquetes en más de un estado actual distinto) y **«Desglose de estados»** (para el popover interactivo).
- **Desglose de estados de consolidados**: **«Paquetes por estado»** es la agrupación operativa dentro del resumen de un consolidado; la acción por grupo es **«Ver paquetes»** (o **«Ver N más»** cuando el preview está truncado) y navega al detalle filtrado. Para paquetes sin estado se usa `sinEstado=true`; no usar `estadoPaqueteId=0`.
...
- **Métricas de estadísticas**:
  ...
  - **«Paquete despachado»**: paquete que avanzó a despacho; se ancla en su **primera transición auditable** cuyo `event_type` es **`ESTADO_APLICADO_DESPACHO`** (`PaqueteEstadoEvento`, `MIN(occurred_at)` por paquete) — un **event_type semántico estable**, NO el id mutable del estado (`estadoRastreoEnDespachoId`) ni `despacho.fecha_hora`. Métrica/serie **«Paquetes despachados»**; el gráfico que las compara es **«Movimiento de paquetes»**. Ya **no** existe un KPI «Despachos» (conteo de entidades `despacho`).
  - **«Peso despachado»**: suma del peso **actual** (`paquete.peso_lbs`) de los paquetes despachados en el periodo (mismo ancla de evento). Es peso actual, no congelado al momento del despacho (puede diferir si se re-pesó); preferencia futura: congelarlo en el evento.
  ...
  - **«Cero real»** vs **«Dato no disponible» / «—»**: un **cero real** (`0` con cobertura **COMPLETA**) se muestra como «0 · Sin actividad registrada en el periodo»; un **dato no disponible** (`null`/sin datos) se muestra como **«—»** con su advertencia. No convertir nulos ni excepciones en cero.
  - **«Disponibilidad de métrica»** (`DisponibilidadMetrica`): **COMPLETA** (cubre todo el periodo) · **PARCIAL** («Datos disponibles desde …», con `coberturaDesde`) · **SIN_CONFIGURACION** («Configura el hito de despacho…») · **SIN_HISTORIAL** («No hay historial suficiente…») · **NO_CALCULABLE**. Aplica a las métricas de despacho (`disponibilidadDespacho`).
...
- **Separación de lenguaje por audiencia (consignatario/destinatario)**: la entidad, tabla, endpoints (`/api/mis-consignatarios`, `/api/operario/consignatarios`), permisos (`CONSIGNATARIOS_*`) y la ruta `/consignatarios` se conservan. Solo cambia el **rótulo visible**: back-office (con `CONSIGNATARIOS_OPERARIO`) ve **Consignatario(s)**; cliente y sesión por enlace ven **Destinatario(s)** (título «Mis destinatarios», CTA «Nuevo destinatario», «Ver envíos»). El rótulo del sidebar se resuelve con `NavItem.labelFor(hasPermission)` en `dashboardNav.ts` sin duplicar la ruta. La misma página `ConsignatarioListPage` y el `ConsignatarioForm` resuelven el copy por audiencia (`esCliente = !hasPermission('CONSIGNATARIOS_OPERARIO')`). Un destinatario puede ser **persona o ubicación receptora** (persona, familiar, colaborador, oficina, sucursal, dirección habitual); el formulario no asume persona natural (label «Nombre del destinatario o ubicación»). El contenido público (home, FAQ, SEO) usa **Destinatario**, nunca «Consignatario»; los archivos públicos están en la allowlist de `lint-nomenclatura.mjs`. Conteos de cliente: **«N guías · N paquetes»** por destinatario (representación única `formatConteosEnvios`), desde proyección backend (`ConsignatarioDTO.totalGuias/totalPaquetes`); CTA **«Ver envíos»** (flecha) en `RecipientShipmentSummary`.
- **Nombre vs. etiqueta del destinatario**: el **nombre** (`Consignatario.nombre`, obligatorio) es **la persona que recibirá** los paquetes (copy cliente: «Nombre de la persona que recibirá», ej. «María López»). La **etiqueta** (`Consignatario.etiqueta`, opcional, texto libre máx. 60, no enum, no única, no genera código) es una **referencia organizativa** para identificar al destinatario (ej. «Oficina», «Regalos», «Compras personales», «Familia», «Trabajo», «Sucursal»). La etiqueta **no sustituye** al nombre, la dirección ni el teléfono y **no** se copia a documentos/snapshots logísticos (sí se versiona en `consignatario_version` como dato editable del maestro). En listas/selectores el nombre es el dato principal y la etiqueta es secundaria.
- **Navegación por audiencia (rótulos de grupos del sidebar)**: fuente única `dashboardNav.ts`. Cliente: grupos **Principal** y **Mis envíos** (Mi casillero · Destinatarios · Mis guías · Mis entregas). Operario: **Inicio y rastreo**, **Gestión de clientes**, **Recepción y transporte**, **Entrega y cierre**, **Red de entrega**. Admin añade **Accesos y seguridad** y **Configuración**. Usuario mixto (operario+cliente, no admin) añade **Mi cuenta** (Mi casillero · Mis guías · Mis entregas). La audiencia se deriva de permisos, no del nombre del rol.
- **Separación de lenguaje por audiencia (entregas/despachos)**: el back-office usa **Despacho** (entidad/salida) y **Tipo de entrega** (enum técnico); la vista de cliente `/mis-entregas` usa **Entrega**, **Mis entregas**, **Entrega #N** (el ID interno del despacho queda como dato secundario, nunca como identificador principal), **Número de rastreo de la entrega** (el `numeroGuia` del despacho), **Modalidad** y **Paquete** (no «Pieza»). Estados de confirmación visibles: **Recibida** (entrega confirmada), **Lista para confirmar** (confirmable) y **En proceso**; KPIs **Entregas / Listas para confirmar / Recibidas**. El cliente no ve «Despacho», «precinto/sacas», «observaciones internas», «liquidación» ni totales globales del despacho. Fuente única de los rótulos de **modalidad** en lenguaje cliente: `ecubox-frontend/src/lib/entregas/modalidad.ts` (consumida por la página, el detalle y los exportadores PDF/XLSX; la capa visual —iconos/badge/búsqueda— vive en `pages/dashboard/mis-entregas/entregaPresentacion.tsx`).
- **Modalidad de entrega (lenguaje cliente)**: `DOMICILIO` → **«Entrega a domicilio»**; `AGENCIA` → **«Retiro en oficina»** (nunca «Agencia» aislada); `AGENCIA_COURIER_ENTREGA` → **«Retiro en punto de entrega»**. (En rastreo público la misma `AGENCIA_COURIER_ENTREGA` se nombra **«Punto de retiro del courier»**; ambas son etiquetas de cliente válidas según el contexto, pero `/mis-entregas` usa «Retiro en punto de entrega».) Nunca se exponen los enums técnicos. El **destino** se resuelve por modalidad con la misma fuente histórica SCD2 que el módulo de despacho.
- **Contenido destacado (campañas de landing)**: en copy se usa **«Campaña»** y **«Contenido destacado»** (sección de Parámetros). Estados visibles: **Borrador**, **Publicada**, **Inactiva**; vigencia derivada **Programada**/**Vigente**/**Vencida**. La campaña es el contenido comercial principal de la landing (tras el Hero); el **tema de temporada** queda como banda secundaria/ambiental (no compiten como dos banners equivalentes ni dos CTA primarios). Solo una campaña **Publicada** a la vez. **Imagen por tema**: **«Imagen · modo claro»** (`imagenUrlClaro`) y **«Imagen · modo oscuro»** (`imagenUrlOscuro`), ambas **opcionales**; **fallback** bidireccional (si falta una se usa la otra) resuelto en frontend según el tema real; **texto alternativo** único. Obligatoriedad por campo: **«Obligatorio»** (nombre interno, tipo), **«Opcional»** (etiqueta, descripción, imágenes, fechas), **«Condicional»** (texto/URL del botón juntos; texto alternativo cuando hay imagen) y **«Obligatorio para publicar»** (título). Un **borrador** puede guardarse incompleto; **publicar** exige los requisitos completos (lista «Para publicar» derivada del schema de publicación).
- **Identidad de marca (rebranding)**: símbolo = **monograma «ec»** enlazado (conexión/infinito), wordmark **ECUBOX** (logo monocromo), lema **«Conecta · Envía · Llega»**; tono limpio, logístico, confiable y trazable. Paleta: negro carbón `#0D0D0F`, gris grafito `#2A2A30`, gris claro `#F2F3F5`, violeta ECUBOX `#6C5CE7`, violeta suave `#EDE9FE`; tipografía **Sora** (SIL OFL). El **icono de la app es carbón** con el símbolo en blanco; el violeta es acento de UI. El logo se usa solo vía `EcuboxLogo` (`@/components/brand`) en UI y vía `src/lib/pdf/brand-logo.ts` en PDFs frontend; no dibujar lockups manuales con `ec`/`ECUBOX`/lema. Los colores solo vía tokens (`--color-*`), nunca hex en componentes. Fuente: `docs/branding/IDENTIDAD_VISUAL.md` y `ecubox-frontend/UI_GUIDELINES.md` §1.6.
...
| Campaña de landing (contenido destacado) | `CampaniaLanding`; público `CampaniaLandingPublicDTO` | `campania_landing` | `/api/parametros-sistema/campanias-landing` (admin), `/api/public/campania-landing` (público) | `/parametros-sistema/contenido-destacado`; landing tras el Hero |
...
- Configuración: `PARAMETROS_SISTEMA_READ`, `ESTADOS_RASTREO_*`, `TARIFA_CALCULADORA_*`, `CONFIG_TARIFA_DISTRIBUCION_*`, `CONTENIDO_DESTACADO_LANDING_READ/WRITE/PUBLISH`, mensajes/canales/temporada.
...
- Evento de rastreo — reparación histórica (`TrackingEventType`): `ESTADO_REPARADO_LOTE_RECEPCION` (auditoría de la herramienta de reparación de "llegada a bodega"; `event_source = REPARACION_LOTE_RECEPCION:<repairRunId>`; **no** genera outbox ni notificaciones). Clasificación de reparación (`PaqueteService.ResultadoReparacionBodega`): `REPARADO`, `YA_CORRECTO`, `YA_REPARADO`, `POSTERIOR`, `ALTERNO`, `BLOQUEADO`, `SIN_FECHA`, `DESTINO_NO_CONFIGURADO`, `NO_ENCONTRADO`.
- Eventos de rastreo escritos por las **migraciones de reconciliación** (`V118`/`V119`/`V120`, SQL directo en `paquete_estado_evento`): `event_source` `MIGRACION_V119` / `MIGRACION_V120`; `idempotency_key` con prefijo `reparacion-historica-v119:` / `reconciliacion-historica-v120:`; reutilizan `event_type` existentes (`ESTADO_APLICADO_*`) más `ESTADO_REPARADO_LOTE_RECEPCION`/`ESTADO_CORREGIDO`/`ESTADO_CONFIRMADO_CLIENTE`. No generan outbox ni notificaciones. Tablas de reporte/auditoría/ambiguos: `migracion_{reporte,auditoria,ambiguos}_v119` y `_v120`.
...
- Campaña de landing — estado (`EstadoCampaniaLanding`): `BORRADOR`, `PUBLICADA`, `INACTIVA`.
- Campaña de landing — tipo (`TipoCampaniaLanding`): `OFERTA`, `INFORMACION`, `NOVEDAD`, `AVISO`.
- Campaña de landing — destino del CTA (`TipoDestinoCta`): `INTERNO`, `EXTERNO`.
- Campaña de landing — **vigencia derivada** (`VigenciaCampaniaLanding`, NO persistida): `PROGRAMADA`, `VIGENTE`, `VENCIDA`.
...
- **Vocabulario de patrones de UI** (no confundir entre sí; ver `UI_GUIDELINES.md` §3 «Bandejas»):
  - **Bandeja**: divide un módulo en **universos de trabajo mutuamente excluyentes** (cambia el dataset base, su contador y su contexto). Componente canónico `@/components/BandejaTabs`; se persiste en URL `?bandeja=`. Ejemplos: Paquetes (`operativos`/`todos`/`en_revision`), Guías master (`operativas`/`pendientes`/`revision`).
  - **Filtro**: **reduce** el universo activo y es combinable (chips/comboboxes): sin peso, vencidos, consignatario, estado, envío, guía. Nunca es una bandeja.
  - **Vista**: forma de **presentar** el mismo universo (tabla vs. tarjetas, escritorio vs. móvil); no cambia el dataset.
  - **Modo**: cambia **cómo** se ejecuta una herramienta local (no cambia el universo): p. ej. distribución de sacas `manual`/`automática`, entrada de guías `lista`/`individual`, «distribuir por» en pesaje, tipo/unidad en enlaces. Se implementa con `SegmentedControl`/tablist local, **no** con `BandejaTabs`.
  - **Paso**: etapa de **captura** dentro de un flujo (stepper); tampoco es bandeja.
- En `/paquetes`, los nombres visibles son **Todos**, **Operativos** y **En revisión**; sus valores API son `todos`, `operativos`, `en_revision`.
- En `/guias-master` hay tres bandejas: **Operativas** (todos los estados salvo `PENDIENTE_VERIFICACION` y `EN_REVISION`), **Pendientes** (rótulo de tab; concepto **Pendiente de aprobación** = `PENDIENTE_VERIFICACION`) y **En revisión** (`EN_REVISION`). El backend segmenta por inclusión de estados (`GET /api/guias-master/page?estado=…`); el frontend añade `bandeja` a la queryKey. Acciones canónicas de la bandeja de aprobación/revisión: **Aprobar** (`/aprobar`, habilita la guía y recalcula), **Enviar a revisión** (`/marcar-en-revision`, **motivo obligatorio**; antes «Marcar/Mandar a revisión») y **Salir de revisión** (`/salir-de-revision`). El **motivo de revisión** es estructurado en la UI (`DATOS_INCONSISTENTES`, `CONSIGNATARIO_INCORRECTO`, `TOTAL_PAQUETES_INCORRECTO`, `NUMERO_GUIA_DUDOSO`, `PAQUETES_REGISTRADOS_ANTES_DE_APROBACION`, `OTRO`; `OTRO` exige observación) y se serializa como `CODIGO: observación` en el campo de texto libre del historial. **Requiere revisión** es la marca por fila de una pendiente con paquetes ya registrados (inconsistencia administrativa; no se corrige automáticamente).
