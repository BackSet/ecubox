...
6. Un lote de recepción registra guías/paquetes que llegan a bodega.
7. Errores relevantes: destino no permitido (incl. `RECIBIDO_EN_BODEGA`), origen sin avance posible (`VACIO`/`LIQUIDADO`/`CANCELADO`/ya en destino), configuración incompleta, duplicado en lote y concurrencia (preview desactualizada).
