...
| Envíos consolidados | `/envios-consolidados`, detalle; `useEnviosConsolidados`, `AvanceEstadosConsolidadosDialog` | `/api/envios-consolidados`; destinos, preview y aplicación en `/estados-destino-secuencia`, `/preview-secuencia-estados` y `/aplicar-secuencia-estados`; `EnvioConsolidadoController` | `EnvioConsolidadoService`, `PaqueteService`, resolver operativo, manifiesto y eventos por cada paso; `envio_consolidado`, `paquete`, `paquete_estado_evento` | `ENVIOS_CONSOLIDADOS_*`; el avance usa `ENVIOS_CONSOLIDADOS_UPDATE` | `EnvioConsolidadoServiceTest`, `EstadoConsolidadoOperativoResolverTest` |
...
2. El avance automático selecciona uno o más consolidados cuyos paquetes comparten un único estado inicial y calcula todos los estados activos posteriores hasta el destino por orden efectivo.
3. `POST /api/envios-consolidados/preview-secuencia-estados` valida la secuencia, fechas y efectos operativos sin modificar datos; devuelve pasos, totales y un `previewToken`.
4. `POST /api/envios-consolidados/aplicar-secuencia-estados` revalida con locks pesimistas y aplica atómicamente cada paso, su evento de rastreo y los efectos operativos configurados.
5. Un token obsoleto produce 409 y obliga a regenerar la vista previa; cualquier fallo revierte la secuencia completa.
6. Un lote de recepción registra guías/paquetes que llegan a bodega.
7. Errores relevantes: estados iniciales mixtos, destino incompatible, cronología inválida, configuración incompleta, duplicado en lote y concurrencia.
