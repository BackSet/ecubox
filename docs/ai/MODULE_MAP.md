...
| Estadísticas | `/estadisticas` (selector de periodo + URL como fuente de verdad vía `validateSearch`); `useEstadisticas`, `PeriodSelector`, `periodo.ts`, `SeriesChart` (gráfico **«Movimiento de paquetes»**: paquetes despachados vs registrados, con estado vacío explícito)/`StatusDistributionChart` | `/api/estadisticas`; `EstadisticasController` | `EstadisticasService`, `PeriodoEstadisticasResolver`, `EstadisticasExcepcionRepository`. **Paquetes despachados** (conteo, serie, peso y tiempo a despacho) se calculan desde la fuente canónica/auditable `PaqueteEstadoEventoRepository` (primera transición —`MIN(occurred_at)`— al estado `estadoRastreoEnDespachoId`), **no** desde `despacho.fecha_hora` (nullable). Registrados desde `PaqueteRepository.aggregateRegistradosByPeriodo`. Se eliminaron `DespachoRepository.aggregateResumen/aggregateByPeriodo` y `PaqueteRepository.avgDiasDespachoEntre`. Financieras = **estimaciones** (peso registrado × tasas históricas) | `ESTADISTICAS_READ` | `EstadisticasServiceTest`, `PeriodoEstadisticasResolverTest`, `EstadisticasCharts.test.tsx`, `periodo.test.ts` |
