...
| Envíos consolidados | `/envios-consolidados`, detalle; `useEnviosConsolidados`, `AplicarEstadoConsolidadosMenuDialog`, `AvanceOperativoConsolidadosDialog` (avance operativo), `AvanceEstadosConsolidadosDialog` (secuencia de rastreo, técnico) | `/api/envios-consolidados`; avance operativo en `/destinos-avance-operativo`, `/preview-avance-operativo`, `/aplicar-avance-operativo`; transición operativa en `/aplicar-transicion-operativa`; estado de rastreo en `/aplicar-estado`; `EnvioConsolidadoController` | `EnvioConsolidadoService`, `PaqueteService`, resolver operativo, manifiesto y eventos por cada paso; `envio_consolidado`, `paquete`, `paquete_estado_evento` | `ENVIOS_CONSOLIDADOS_*`; el avance usa `ENVIOS_CONSOLIDADOS_UPDATE` | `EnvioConsolidadoServiceTest`, `EstadoConsolidadoOperativoResolverTest`, `AvanceOperativoConsolidadosDialog.test.tsx` |
...
| Rastreo público | `/tracking`, `/tracking/ejemplo`; `tracking.service.ts`, componentes de resultados/exportación (`TrackingActionsBar` + `components/export/ExportTile` compartido con la calculadora) | `/api/tracking`; `TrackingController`, health de proyector | `TrackingResolverService`, eventos/outbox/proyector; `paquete_estado_evento`, `outbox_event`, `tracking_view_paquete`, `tracking_projector_state` | Público con rate limit; health interno con `TRACKING_PROJECTOR_HEALTH_READ` | Resolver, eventos, proyector, ETag, cache y rate limit |
...
2. **«Aplicar estado»** ofrece tres modos independientes (`AplicarEstadoConsolidadosMenuDialog`): **Transición operativa** (un paso operativo, `POST /aplicar-transicion-operativa`), **Avance automático** (varios pasos operativos hasta un destino) y **Estado de rastreo de paquetes** (acción técnica sobre paquetes, `POST /aplicar-estado`).
3. El **avance automático** opera sobre `EstadoEnvioConsolidadoOperativo` (NO sobre estados de rastreo): destinos permitidos `CERRADO`, `ENVIADO_DESDE_USA`, `ARRIBADO_ECUADOR`. `GET /api/envios-consolidados/destinos-avance-operativo` lista destinos; `POST /preview-avance-operativo` calcula los pasos operativos intermedios y un `previewToken`; `POST /aplicar-avance-operativo` revalida con locks pesimistas y aplica atómicamente cada transición operativa (cerrar, enviar desde USA, arribar) reutilizando sus efectos. `RECIBIDO_EN_BODEGA` no es destino ni origen del avance (se asigna al registrar el consolidado en un lote de recepción).
4. Un token obsoleto produce 409 y obliga a regenerar la vista previa; cualquier fallo revierte la operación completa.
5. Las rutas de secuencia por estado de rastreo (`/estados-destino-secuencia`, `/preview-secuencia-estados`, `/aplicar-secuencia-estados`) siguen existiendo para usos técnicos, pero el «Avance automático» de la UI ya no las usa.
6. Un lote de recepción registra guías/paquetes que llegan a bodega. **Admisión por estado anterior**: solo se pueden recibir consolidados en `ARRIBADO_ECUADOR`. `findDisponiblesParaRecepcion` (y la búsqueda) listan únicamente consolidados en ese estado, con paquetes y no recibidos antes; `LoteRecepcionService.create`/`agregarGuias` revalidan transaccionalmente y rechazan con error claro cualquier consolidado fuera de `ARRIBADO_ECUADOR` aunque el frontend lo envíe manualmente. Al admitir uno válido, el consolidado pasa explícitamente a `RECIBIDO_EN_BODEGA` y a sus paquetes se les aplica el estado configurado por punto para lote de recepción (sin hardcodear).
7. Errores relevantes del avance: destino no permitido (incl. `RECIBIDO_EN_BODEGA`), origen sin avance posible (`VACIO`/`LIQUIDADO`/`CANCELADO`/ya en destino), configuración incompleta, duplicado en lote y concurrencia (preview desactualizada).
8. **Despacho** mantiene la misma regla: las sacas solo ingresan si todos sus paquetes están exactamente en el estado anterior a `estadoRastreoEnDespachoId` (`DespachoService.validarSacasNuevas`); los paquetes entran solo vía sacas, sin bypass directo.
