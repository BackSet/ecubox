...
-- Nota: el backfill del evento de "entrada a despacho" para estadísticas se
-- movió a la migración versionada V114 (db/migration), que corre en TODOS los
-- entornos de forma idempotente y deja los eventos reconstruidos correctamente
-- identificados (event_source='BACKFILL_DESPACHO'). Ya no se reconstruye aquí.
